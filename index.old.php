...
if ($page==='profile') {
    ...
    if ($action==='change_password' && $_SERVER['REQUEST_METHOD']=== 'POST') {
        ...
        elseif($cu['password']!==md5($cur)) $_SESSION['pwd_error']='Mật khẩu hiện tại không đúng';
        ...
        elseif(md5($np)===$cu['password']) $_SESSION['pwd_error']='Mật khẩu mới phải khác mật khẩu cũ';
...
if ($page==='imports' && $_SERVER['REQUEST_METHOD']=== 'POST' && ($_POST['action']??'')=== 'import') {
    requireRole(['superadmin','admin','warehouse']);
...
if ($page==='users') {
    ...
    if ($action==='toggle'){$r=toggleUserActive($_GET['username']??'');$_SESSION['flash']=['type'=> $r['success']?'success':'danger','message'=> $r['message']];header('Location: index.php?page=users');exit;}
    if ($action==='delete'){$r=deleteUser($_GET['username']??'');$_SESSION['flash']=['type'=> $r['success']?'success':'danger','message'=> $r['message']];header('Location: index.php?page=users');exit;}
}
...
if ($page==='backup') {
    ...
    if ($action==='delete'){$f=basename($_GET['file']??'');$p=$bp.'/'.$f;if($f&&file_exists($p)&&str_starts_with($f,'backup_')){unlink($p);$_SESSION['flash']=['type'=>'success','message'=> "Đã xóa: {$f}"];}header('Location: index.php?page=backup');exit;}
    if ($action==='cleanup'){$files=glob($bp.'/backup_*.zip')??[];usort($files,fn($a,$b)=>filemtime($b)<=>filemtime($a));$d=0;foreach(array_slice($files,10) as $f){unlink($f);$d++;}$_SESSION['flash']=['type'=>'success','message'=> "Đã xóa {$d} file cũ"];header('Location: index.php?page=backup');exit;}
}
